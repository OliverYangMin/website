local function Csv2Matrix(data, ifhead)  --read_csv, specified for this problem
 ...
 local function Disrupted(cRotations)  ---determine flight is disrupted
    if     cRotations[1].port1==57 and cRotations[1].time1<600 and cRotations[1].time1>=480 then
        return 1
    elseif cRotations[1].port2==57 and cRotations[1].time2<600 then 
        return 2
    end 
    return false
end     

local function GetFlights()  ---load all information from filghts.csv   get flights, aircrafts, airports
...
local function GetAirports() --get the airports from flights and their information
...
local function GetAircrafts() --get aircrafts worked an idle, and their information
    local file1 = 'data\\BD_idle.csv'
    local file2 = 'data\\BD_aircrafts_water.csv'
   
    aircrafts = {}       ---craft = {id=, rot={}, start=, base={}, water=, }
    ...
    for c, craft in pairs(aircrafts) do
        ...
        if dis==1 then
            ...
            craft.rot[1].time2 = craft.rot[1].time2 + 120--OTIME1 - CTIME1
            craft.time1 = 600--OTIME1 
        elseif dis==2 then
            craft.dis = true
            craft.rot[1].dis   = true
    ...
    for i=1,#Inf do
        if not aircrafts[Inf[i][2]] then 
            aircrafts[Inf[i][2]] = {start=Inf[i][4], atp=Inf[i][3], seats=Inf[i][5], id=Inf[i][2], rot={}, water=true, base={}}
